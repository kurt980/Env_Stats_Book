...
-   **Support Vector**: These are the data points that lie closest to the decision surface (hyperplane) and are critical in defining the position and orientation of the hyperplane. These points satisfy:
...
In a linearly separable case, it's possible to find a hyperplane that perfectly separates the classes such that:
...
The objective function $\frac{1}{2} \|\mathbf{w}\|^2$ minimizes the norm of the weight vector, which is equivalent to maximizing the margin (since the margin is inversely proportional to $\|\mathbf{w}\|$). This is a quadratic programming problem. In practice, SVM uses Lagrange multipliers to transform this into a problem that involves only the dot products of samples, facilitating the use of the kernel trick.

-   **Dual Formulation**

To simplify solving this optimization problem, especially when using the kernel trick, SVM formulations often convert this "primal" problem into its "dual" form. The dual problem focuses on maximizing a Lagrangian based on conditions derived from the primal problem:
