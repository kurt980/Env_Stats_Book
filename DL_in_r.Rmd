...
combined_data$Time <- as.Date(as.yearmon(combined_data$Time, "%Y %b"))
...
### Create Lags for variables
```{r}
# Create lagged features for NAO, AMO, ONI, and PDO
...
```{r}
# Assuming combined_data is already prepared and contains multiple variables including 'Time'
# First, we'll remove the 'Time' column from standardization if it's a Date object

# Select numeric columns only (excluding Date if necessary)
numeric_data <- combined_data %>% select_if(is.numeric)

# Apply standardization
standardized_data <- scale(numeric_data)

# Convert back to a data frame and add the 'Time' column back if it was excluded
standardized_data <- as.data.frame(standardized_data)
standardized_data$Time <- combined_data$Time

# Verify the structure and see the first few rows
